...
class articleC {
    ...
    function ajouterarticle ($a){
        ...
        $sql="insert into article (date_a,titre,editor1) values 
(:date_a, :titre, :editor1 )";
        $conn = NULL;
        try{
            echo"1111";
            $conn = new PDO("mysql:host=localhost;dbname=agence_de_voyage",
    ...
    function modifierarticle($e,$date_a){
        ...
        try{
            ...
            $image=$e->getimage();
                ...
                $sql="UPDATE article SET date_a='$date_a', titre='$titre', editor1='$editor1' WHERE date_a='$date_a'";
                     $req = $this->db->prepare($sql);
    ...
    public function supprimer_a($titre)
    {
        ...
        $this->db = $conn;
        $sql= "SELECT * FROM article ORDER BY date_a ";
        try{
            $liste=$this->db->query($sql);
            return $liste;
        }
